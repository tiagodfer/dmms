...
public class DMMS {
    public static void main (String args[]) {
        if (args.length >= 7) {
            ...
            Deallocator deallocator = new Deallocator(MIN_THRESHOLD, heap);
            Defragger defragger = new Defragger(heap);
            Allocator allocator = new Allocator(MAX_OCCUPATION, MAX_FRAGMENTATION, REQ_NUMBER, lock, empty, full, deallocator, defragger, queue, heap);
